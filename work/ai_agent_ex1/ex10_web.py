"""
웹 기반으로 서비스를 구현해보자!
pip install test "fast[standard]"
    ...
    html_content = f'''
        ...
        <body>
            ...
            <form action="/chat" method="post">
                <input type="text" name="user_message" placeholder="메세지를 입력" required/>
...
persona_prompt = (
    ...
    "항상 간결하게 답변해줘 길어야 두 세 문장으로 답변해주고, 어린 왕자의 순수함과 지혜를 반영해줘. 복잡한 주제도 본질적으로 단순화하여 설명해줘"
)
...
def chatbot_response(msg):
    response = client.messages.create(
        ...
        max_tokens=1024,
        system=persona_prompt, # system파라미터로 프롬프트 유형 전달
        messages=msg
    )
...
# 용어 하나만 익히기: RAG (Retrieval-Augmented Generation) - 검색 증강 생성
