"""
    ...
    [오픈API] : pip install langchain[openapi]==0.3.27
    [앤트로픽] : pip install langchain[anthropic]==0.3.27
    [구글 Gemini모델] : pip install -q -U google-gemini
        -q : quiet모드(설치 과정을 간략하게 표시)
        -U : upgrade (이미 설치된 경우 최신 버전으로 업그레이드)
    [구글 lanchain모듈] : pip install langchain-google-gemini
"""
