"""
LLM을 사용할 때 가끔은 실패할때가 있다. 이때 재시도를 하기위해
다음과 같은 패키지를 설치하고 재시도를 수행하도록 유도해야 함
...
@retry(
    ...
    before_sleep=lambda retry_state: logger.warning(
        f"API호출 실패: {retry_state.outcome.exception()}, " # 포맷을 설정하여 구문 처리(처리할 구문을 중괄호로 묶고 맨 앞에 f 명시해줘야함)
...
async def main():
    ...
    try:
        ...
        print("Claude API호출 완료: {claude_response[0]}")
